...
CClientManager::CClientManager() :
	...
	m_iAttendanceFlushDelaySec(3600)
#endif
{
	m_itemRange.dwMin = 0;
	m_itemRange.dwMax = 0;
...
#if defined(__ATTENDANCE_SYSTEM__)
	CClientManager::instance().FlushAttendance();
#endif
...
void CClientManager::QUERY_EMPIRE_SELECT(CPeer * pkPeer, DWORD dwHandle, TEmpireSelectPacket * p)
{
	...
	sys_log(0, "EmpireSelect: %s", szQuery);
	{
		...
		if (pRes->uiNumRows)
		{
			...
			{
				{ 520400, 459600 },	// �ż��� Rossi
				{ 388100, 439300 },	// õ���� gialli
				{ 526500, 572900 }	// ���뱹 Blu
			};
...
void CClientManager::QUERY_ITEM_SAVE(CPeer * pkPeer, const char * c_pData)
{
	...
	// â���� ĳ������ �ʰ�, ĳ���� �ִ� �͵� �������� �Ѵ�.
	// auction�� �� ��Ʈ�� Ÿ�� �ʾƾ� �Ѵ�. EnrollInAuction�� Ÿ���Ѵ�.
...
#endif
#if defined(__ATTENDANCE_SYSTEM__)
		case HEADER_GD_ATTENDANCE_LOAD:
			LoadAttendance(peer, dwHandle, (TAttendanceTable*)data);
			break;

		case HEADER_GD_ATTENDANCE_SAVE:
			SaveAttendance(peer, dwHandle, (TAttendanceTable*)data);
			break;

		case HEADER_GD_ATTENDANCE_DUMP:
			FlushAttendance(true);
			break;
#endif
			default:
...
#if defined(__ATTENDANCE_SYSTEM__)
			if (!(thecore_heart->pulse % (thecore_heart->passes_per_sec * m_iAttendanceFlushDelaySec)))
				CClientManager::instance().FlushAttendance();
#endif
...
void CClientManager::UnblockHwid(THwidRequest * pack) {
	...
	void CClientManager::RedeemPromotionCode(CPeer* pkPeer, DWORD dwHandle, TPromotionRedeem* p)
	{
		...
		if (m_map_promotionCodeInfo.find(p->code) != m_map_promotionCodeInfo.end() && m_map_promotionCodeInfo[p->code].first > 0)
		{
			...
			m_map_redeemedPromotionFlush.insert(std::make_pair(p->accID, p->code));
...
#endif
#if defined(__ATTENDANCE_SYSTEM__)
void CClientManager::LoadAttendance(CPeer* pPeer, DWORD dwHandle, const TAttendanceTable* pkTable)
{
	TAttendanceTable AttendanceTable = {};
	AttendanceCacheMap::iterator it = m_map_AttendanceCacheMap.find(pkTable->dwPID);
	if (it != m_map_AttendanceCacheMap.end())
	{
		AttendanceTable = it->second;
	}
	else
	{
		AttendanceTable.dwPID = pkTable->dwPID;

		char szQuery[128 + 1] = {};
		snprintf(szQuery, sizeof(szQuery), "SELECT * FROM `attendance%s` WHERE `pid` = %u",
			GetTablePostfix(), pkTable->dwPID);
		std::unique_ptr<SQLMsg> pMsg(CDBManager::instance().DirectQuery(szQuery));

		const SQLResult* pkRes = pMsg->Get();
		if (pkRes->uiNumRows > 0)
		{
			MYSQL_ROW cRow = mysql_fetch_row(pkRes->pSQLResult);
			BYTE bCol = 1; // skip `pid` field
			for (BYTE bDay = 0; bDay < ATTENDANCE_DAYS; bDay++)
				str_to_number(AttendanceTable.bState[bDay], cRow[bCol++]);
		}
		else
			std::memset(AttendanceTable.bState, 0, sizeof(AttendanceTable.bState));

		m_map_AttendanceCacheMap.emplace(std::make_pair(pkTable->dwPID, AttendanceTable));
	}

	pPeer->EncodeHeader(HEADER_DG_ATTENDANCE_LOAD, dwHandle, sizeof(AttendanceTable));
	pPeer->Encode(&AttendanceTable, sizeof(AttendanceTable));
}

void CClientManager::SaveAttendance(CPeer* pPeer, DWORD dwHandle, const TAttendanceTable* pkTable)
{
	AttendanceCacheMap::iterator it = m_map_AttendanceCacheMap.find(pkTable->dwPID);
	if (it != m_map_AttendanceCacheMap.end())
	{
		...
		m_map_AttendanceCacheMap.emplace(std::make_pair(pkTable->dwPID, *pkTable));
}

void CClientManager::FlushAttendance(bool bFlush)
{
	if (bFlush)
		CDBManager::instance().DirectQuery("TRUNCATE TABLE `attendance`");

	for (const auto& rkAttendanceCacheMap : m_map_AttendanceCacheMap)
	{
		const TAttendanceTable& rkAttendanceTable = rkAttendanceCacheMap.second;
		if (rkAttendanceCacheMap.first != rkAttendanceTable.dwPID)
			continue;
	
		char szQuery[2048 + 1] = {};
		int iLen = 0;

		iLen = sprintf(szQuery, "REPLACE INTO `attendance` (`pid`");
		for (BYTE bDay = 0; bDay < ATTENDANCE_DAYS; ++bDay)
			iLen += sprintf(szQuery + iLen, ", `day%d`", bDay + 1);

		iLen += sprintf(szQuery + iLen, ") VALUES (%d", rkAttendanceTable.dwPID);
		for (BYTE bDay = 0; bDay < ATTENDANCE_DAYS; ++bDay)
			iLen += sprintf(szQuery + iLen, ", %d", rkAttendanceTable.bState[bDay]);
	...
	if (bFlush)
		m_map_AttendanceCacheMap.clear();
}
#endif
