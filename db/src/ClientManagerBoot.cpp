...
bool CClientManager::InitializeTables()
{
	...
	if (!InitializeAttendanceTable())
	{
		sys_err("InitializeAttendanceTable FAILED");
		return false;
	}
#endif
...
{
	char query[4096];
	snprintf(query, sizeof(query), "SELECT apply, apply+0, prob, lv1, lv2, lv3, lv4, lv5, weapon, body, wrist, foots, neck, head, shield, ear"
									...
									", costume_stole"
#endif
									", lv1_costume, lv2_costume, lv3_costume, lv4_costume, lv5_costume"
#endif
									" FROM item_attr%s ORDER BY apply", GetTablePostfix());
...
#ifdef ENABLE_STOLE_COSTUME
		str_to_number(t.bMaxLevelBySet[ATTRIBUTE_SET_COSTUME_STOLE], data[col++]);
#endif
...
bool CClientManager::MirrorMobTableIntoDB()
{
	...
	for (itertype(m_vec_mobTable) it = m_vec_mobTable.begin(); it != m_vec_mobTable.end(); it++)
	{
		...
		{
			len += snprintf(query + len, sizeof(query)-len, "locale_name_%s, ", languages[i].c_str());
		}
		len += snprintf(query + len, sizeof(query) - len, "type, rank, battle_type, level, size, ai_flag, setRaceFlag, setImmuneFlag, "
			...
			"%d, \"%s\", ", t.dwVnum, t.szName);

		for (int i = 0; i < LANGUAGE_MAX_NUM; ++i)
			len += snprintf(query + len, sizeof(query) - len, "\"%s\", ", t.szLocaleName[i]);
	...
	for (itertype(m_vec_itemTable) it = m_vec_itemTable.begin(); it != m_vec_itemTable.end(); it++)
	{
		if (g_stLocaleNameColumn != "name")
		{
			...
			snprintf(query, sizeof(query),
				...
				t.dwVnum, t.bType, t.bSubType, t.szName,
#ifdef ENABLE_MULTI_NAMES
				t.szLocaleName[0],
#else
				t.szLocaleName,
#endif
				t.dwGold, t.dwShopBuyPrice, t.bWeight, t.bSize, //11
		...
		else
		{
			...
			snprintf(query, sizeof(query),
				...
				GetTablePostfix(),
				t.dwVnum, t.bType, t.bSubType, t.szName, 
				t.dwGold, t.dwShopBuyPrice, t.bWeight, t.bSize,
...
enum MProtoT
{
	vnum, name,
#ifdef ENABLE_MULTI_NAMES
	locale_name_en, locale_name_ro, locale_name_it, locale_name_tr, locale_name_de, locale_name_pl, locale_name_pt, locale_name_es, locale_name_cz, locale_name_hu,
#else
	locale_name,
#endif
...
bool CClientManager::InitializeMobTableFromDB()
{
	...
	iQueryLen += snprintf(query + iQueryLen, sizeof(query) - iQueryLen, "type, rank, battle_type, level, size+0,"
		...
		" skill_vnum0, skill_level0, skill_vnum1, skill_level1, skill_vnum2, skill_level2, skill_vnum3, skill_level3,"
		" skill_vnum4, skill_level4, sp_berserk, sp_stoneskin, sp_godspeed, sp_deathblow, sp_revive"
		" FROM mob_proto%s ORDER BY vnum;", GetTablePostfix());
#else
	snprintf(query, sizeof(query),
...
#ifdef ENABLE_MULTI_NAMES
		VERIFY_SFIELD(MProto::locale_name_en, mob_table->szLocaleName[1]);
		VERIFY_SFIELD(MProto::locale_name_ro, mob_table->szLocaleName[2]);
		VERIFY_SFIELD(MProto::locale_name_it, mob_table->szLocaleName[3]);
		VERIFY_SFIELD(MProto::locale_name_tr, mob_table->szLocaleName[4]);
		VERIFY_SFIELD(MProto::locale_name_de, mob_table->szLocaleName[5]);
		VERIFY_SFIELD(MProto::locale_name_pl, mob_table->szLocaleName[6]);
		VERIFY_SFIELD(MProto::locale_name_pt, mob_table->szLocaleName[7]);
		VERIFY_SFIELD(MProto::locale_name_es, mob_table->szLocaleName[8]);
		VERIFY_SFIELD(MProto::locale_name_cz, mob_table->szLocaleName[9]);
		VERIFY_SFIELD(MProto::locale_name_hu, mob_table->szLocaleName[10]);
#else
...
bool CClientManager::InitializeItemTableFromDB()
{
	...
	iQueryLen += snprintf(query + iQueryLen, sizeof(query) - iQueryLen, "gold, shop_buy_price, weight, size,"
		...
		" , vnum_range"
#endif
		" FROM item_proto%s ORDER BY vnum;", GetTablePostfix());
#else
	snprintf(query, sizeof(query),
...
#ifdef ENABLE_MULTI_NAMES
		VERIFY_SFIELD(IProto::locale_name_en, item_table->szLocaleName[1]);
		VERIFY_SFIELD(IProto::locale_name_ro, item_table->szLocaleName[2]);
		VERIFY_SFIELD(IProto::locale_name_it, item_table->szLocaleName[3]);
		VERIFY_SFIELD(IProto::locale_name_tr, item_table->szLocaleName[4]);
		VERIFY_SFIELD(IProto::locale_name_de, item_table->szLocaleName[5]);
		VERIFY_SFIELD(IProto::locale_name_pl, item_table->szLocaleName[6]);
		VERIFY_SFIELD(IProto::locale_name_pt, item_table->szLocaleName[7]);
		VERIFY_SFIELD(IProto::locale_name_es, item_table->szLocaleName[8]);
		VERIFY_SFIELD(IProto::locale_name_cz, item_table->szLocaleName[9]);
		VERIFY_SFIELD(IProto::locale_name_hu, item_table->szLocaleName[10]);
#else
		VERIFY_SFIELD(MProto::locale_name, mob_table->szLocaleName);
#endif
...
#if defined(__ATTENDANCE_SYSTEM__)
bool CClientManager::InitializeAttendanceTable()
{
	char szQuery[128 + 1];
	snprintf(szQuery, sizeof(szQuery), "SELECT * FROM `player`.`attendance`");
	...
	MYSQL_ROW cRow;
	while ((cRow = mysql_fetch_row(pkRes->pSQLResult)))
	{
		BYTE bCol = 0;

		TAttendanceTable AttendanceTable;
		str_to_number(AttendanceTable.dwPID, cRow[bCol++]);

		for (BYTE bDay = 0; bDay < ATTENDANCE_DAYS; bDay++)
			str_to_number(AttendanceTable.bState[bDay], cRow[bCol++]);

		m_map_AttendanceCacheMap.emplace(std::make_pair(AttendanceTable.dwPID, AttendanceTable));
	}
