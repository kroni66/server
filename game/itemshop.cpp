	...
	bool CItemshopManager::CanBuyItem(LPCHARACTER ch, const char* hash, WORD wCount)
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "You have to wait 10 seconds after an invalid itemshop action.");
			return false;
		}
		...
		{
			ch->SetItemshopCooldown(thecore_pulse() + PASSES_PER_SEC(10));
			ch->ChatPacket(CHAT_TYPE_INFO, "You dont have enough coins.");
			return false;
		}
		...
		if (!item)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "Invalid item -> Write your Serveradmin to get some free coins :)");
			return false;
		}
		...
		if (item->IsDragonSoul())
		{
			...
			{
				ch->ChatPacket(CHAT_TYPE_INFO, "You carry too many Items.");
	...
	bool CItemshopManager::BuyItem(LPCHARACTER ch, const char* hash, WORD wCount)
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, "The purchase of the item was successful.");
