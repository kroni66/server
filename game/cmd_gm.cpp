...
ACMD(do_attendance)
{
	...
	if (!*szArg1 && !*szArg2)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Usage: /attendance <state> <month>");
		ch->ChatPacket(CHAT_TYPE_INFO, "    <state> : 0 = disable, 1 = enable");
		ch->ChatPacket(CHAT_TYPE_INFO, "    <month> : 0 = auto, 1 = january, ...");
		return;
	}

	BYTE bEnable = 1, bMonth = 1;
	if (isnhdigit(*szArg1) && isnhdigit(*szArg2))
	{
		str_to_number(bEnable, szArg1);
		str_to_number(bMonth, szArg2);
	}

	std::time_t lCurrentTime = std::time(nullptr);
	if (bMonth == 0)
		bMonth = std::localtime(&lCurrentTime)->tm_mon + 1;

	const CAttendanceGroup* pkGroup = ITEM_MANAGER::instance().GetAttendanceGroup(bMonth);
	if (pkGroup == nullptr)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "<Attendance> There are no items for that month. (%d)", bMonth);
		return;
	}
	...
	switch (bEnable)
	{
		case 1:
		{
			if (quest::CQuestManager::instance().GetEventFlag("attendance") > 0)
			{
				ch->ChatPacket(CHAT_TYPE_INFO, "<Attendance> Attendance is already ongoing.");
				return;
			}

			long lDuration = (test_server ? 60 * ATTENDANCE_DAYS : static_cast<long>(ATTENDANCE_DURATION));
			quest::CQuestManager::instance().RequestSetEventFlag("attendance", 1);
			quest::CQuestManager::instance().RequestSetEventFlag("attendance_begin", lCurrentTime);
			quest::CQuestManager::instance().RequestSetEventFlag("attendance_end", lCurrentTime + lDuration);
			quest::CQuestManager::instance().RequestSetEventFlag("attendance_month", bMonth);
			ch->ChatPacket(CHAT_TYPE_INFO, "<Attendance> The attendance rewards are now enabled.");
		}
		break;
		...
		default:
		{
			quest::CQuestManager::instance().RequestSetEventFlag("attendance", 0);
			quest::CQuestManager::instance().RequestSetEventFlag("attendance_begin", 0);
			quest::CQuestManager::instance().RequestSetEventFlag("attendance_end", 0);
			quest::CQuestManager::instance().RequestSetEventFlag("attendance_month", 0);
			ch->ChatPacket(CHAT_TYPE_INFO, "<Attendance> The attendance rewards are now disabled.");
