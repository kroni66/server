...
void CInputLogin::CharacterPinCode(LPDESC d, const char* c_pData)
{
	...
	if (CharacterPinCodeCGPacket.bIndex >= PLAYER_PER_ACCOUNT)
	{
	...
	if (strcmp(c_rAccountTable.players[CharacterPinCodeCGPacket.bIndex].szPinCode, CharacterPinCodeCGPacket.szPinCode) != 0)
	{
		...
		// PLAYER_PIN_CODE_RESULT
		{
			...
			snprintf(szQuery, sizeof(szQuery),
				"SELECT pin FROM player.player WHERE id=%u AND account_id=%u", c_rAccountTable.players[CharacterPinCodeCGPacket.bIndex].dwID, c_rAccountTable.id);
		...
		{
			CharacterPinCodeGCPacket.bVerified = false;
			d->DelayedDisconnect(3);
		}
		else
		{
			CharacterPinCodeGCPacket.bVerified = true;
		}
	}
	else
	{
