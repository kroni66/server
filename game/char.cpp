...
void CHARACTER::Initialize()
{
	...
	m_pkAttendance = nullptr;
	m_bIsOpenAttendance = false;
#endif
...
void CHARACTER::SendFlyData(long target_fly, DWORD target_vid)
{
	TPacketNewMovement packet;
	packet.header = HEADER_GC_NEW_MOVEMENTS; //szerver -> kliens
	packet.type = 0;
	packet.val = target_fly;
	packet.vid = target_vid;
	PacketAround(&packet, sizeof(TPacketNewMovement));
}
#endif
...
void CHARACTER::SendTargetDrop()
{
	...
	if (test_server)
		ChatPacket(CHAT_TYPE_INFO, "TPacketGCTargetDrop");

	TPacketGCTargetDrop p;
	p.header = HEADER_GC_TARGET_DROP;
	p.raceVnum = m_pkChrTarget->GetRaceNum();
	p.size = 0;

	if (!m_map_drop_item.empty())
		m_map_drop_item.clear();

	if (ITEM_MANAGER::instance().GetDropItem(m_pkChrTarget, m_map_drop_item)) {
		if (m_map_drop_item.size()) {
			p.size = m_map_drop_item.size();
			for (DWORD i = 0; i < MIN(ITEM_DROP_SIZE, m_map_drop_item.size()); ++i)
				p.items[i] = m_map_drop_item[i];
		}
	}
...
void CHARACTER::EncodeInsertPacket(LPENTITY entity) {
	...
	if (ch->FlyState)
	{
		SendFlyData(ch->FlyHeight, static_cast<DWORD>(ch->GetVID()));
	}
#endif
...
#if defined(__ATTENDANCE_SYSTEM__)
void CHARACTER::SetAttendance(CAttendance* pAttendance)
{
	if (m_pkAttendance)
		delete m_pkAttendance;

	m_pkAttendance = pAttendance;
}
#endif