...
namespace quest
{
	...
	ALUA(pc_ore_refine)
	{
		...
		LPITEM metinstone_item = ch->GetExtraInventoryItem(metinstone_cell);
#else
...
#ifdef __PLAYER_PIN_SYSTEM__
	int pc_change_pin(lua_State* L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		...
		{
			sys_err("QUEST change_pin invalid argument.");
			return 0;
		}

		const std::string c_strOldPinCode = lua_tostring(L, 1);
		const std::string c_strNewPinCode = lua_tostring(L, 2);

		if (c_strOldPinCode.length() != 4 || !c_strNewPinCode.length() != 4)
		{
			sys_err("QUEST change_pin invalid argument length.");
			return 0;
		}

		char szQuery[QUERY_MAX_LEN];
		snprintf(szQuery, sizeof(szQuery), "SELECT COUNT(*) as count FROM player%s WHERE pin = '%s' AND id = %u", get_table_postfix(), c_strOldPinCode.c_str(), ch->GetPlayerID());
		std::unique_ptr<SQLMsg> pMsg(DBManager::instance().DirectQuery(szQuery));
		if (pMsg->Get()->pSQLResult && pMsg->Get()->uiNumRows > 0)
		{
			MYSQL_ROW row = mysql_fetch_row(pMsg->Get()->pSQLResult);
			BYTE bResult = 0;
			...
			if (bResult)
			{
				...
				sys_log(0, "[ChangePIN] %s : %s -> %s", ch->GetName(), c_strOldPinCode.c_str(), c_strNewPinCode.c_str());
				char szUpdateQuery[QUERY_MAX_LEN];
				snprintf(szUpdateQuery, sizeof(szUpdateQuery), "UPDATE player%s SET pin = '%s' WHERE id = %u", get_table_postfix(), c_strNewPinCode.c_str(), ch->GetPlayerID());
				DBManager::Instance().DirectQuery(szUpdateQuery);

				ch->ChatPacket(CHAT_TYPE_INFO, "You have changed your PIN code.");
				lua_pushboolean(L, true);
				return 1;
			}
		}
