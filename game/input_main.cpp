...
void CInputMain::TargetInfoLoad(LPCHARACTER ch, const char* c_pData)
{
	...
	sys_log(0, "Received TARGET_INFO_LOAD packet. dwVID: %d", p->dwVID);
	...
	sys_err(0,"Nasli jsme cilovy dwVID..." );

	if (!ch || !m_pkChrTarget)
		return;
	sys_err(0,"Overili jsme jestli je charakter a cilovy charakter..." );
	...
	if (ITEM_MANAGER::instance().CreateDropItemVector(m_pkChrTarget, ch, s_vec_item) && (m_pkChrTarget->IsMonster() || m_pkChrTarget->IsStone()))
	{
		...
		else if (s_vec_item.size() == 1)
		{
			...
			sys_err(0,"Paket odeslan zpet klientovi splnena podminka." );
		}
		else
		{
			...
			while (iItemIdx >= 0)
			{
				...
				if (!pkInfoItem)
				{
					sys_err(0,"pkInfoItem null in vector idx %d", iItemIdx + 1);
					continue;
				}
				...
				ch->GetDesc()->Packet(&pInfo, sizeof(TPacketGCTargetInfo));
				sys_err(0,"Paket odeslan zpet klientovi nesplnena podminka." );
			}
		}
	}
}
...
//		len = snprintf(chatbuf, sizeof(chatbuf), "|L%s|l%s |Hmsg:%s|h%s [PM]|h|r : %s", langName.c_str(), (ch->IsGM()?colorbuf[0]:colorbuf[MINMAX(0, ch->GetEmpire(), 3)]), ch->GetName(), ch->GetName(), buf);
		len = snprintf(chatbuf, sizeof(chatbuf), "|L%s|l|E%d|e |Hmsg:%s|h%s [PM]|h|r : %s", langName.c_str(), ch->GetEmpire(), ch->GetName(), ch->GetName(), buf);
	} else {
...
void CInputMain::Attendance(LPCHARACTER pkChar, const char* c_pszData)
{
	const TPacketCGAttendance* pkPacket = reinterpret_cast<const TPacketCGAttendance*>(c_pszData);
	if (pkPacket == nullptr)
		return;

	switch (pkPacket->bSubHeader)
	{
		case SUBHEADER_ATTENDANCE_CLOSE:
			CAttendance::Close(pkChar);
			break;

		case SUBHEADER_ATTENDANCE_OPEN:
			CAttendance::Open(pkChar);
			break;

		case SUBHEADER_ATTENDANCE_COLLECT:
			CAttendance::Receive(pkChar);
		break;

		default:
			sys_err("CInputMain::Attendance Unkown SubHeader (ch: %s) (%d)", pkChar->GetName(), pkPacket->bSubHeader);
			break;
	}
}
#endif
...
#ifdef FLY_SYSTEM
		case HEADER_GC_NEW_MOVEMENTS:
			RecvNewMovements(ch, c_pData);
			break;
#endif
...
#if defined(__ATTENDANCE_SYSTEM__)
		case HEADER_CG_ATTENDANCE:
			Attendance(ch, c_pData);
			break;
#endif
...
#ifdef DROP_WIKI
void CInputMain::RequestDropItem(LPCHARACTER ch)
{
	int iPulse = thecore_pulse();

	if (iPulse - ch->GetItemDropRecvTime() < passes_per_sec * 3)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "RECV_ITEM_DROP_WAIT");
		return;
	}
	ch->SetItemDropRecvTime(iPulse);
	ch->SendTargetDrop();
...
#ifdef FLY_SYSTEM
void CInputMain::RecvNewMovements(LPCHARACTER ch, const char* c_pData)
{
	TPacketNewMovement* p = (TPacketNewMovement*)c_pData;
	TPacketNewMovement packet;
	packet.header = HEADER_GC_NEW_MOVEMENTS; //szerver -> kliens
	packet.type = p->type;
	packet.val = p->val;

	//LPCHARACTER mainTarget = CHARACTER_MANAGER::instance().Find(ch->GetVID());

	if (packet.type == 1) //flyoff
	{
		if (p->vid == ch->GetVID()) //igy nem tudnak mas jatekosok sebzest okozni
		{
			ch->FlyState = false;
			ch->FlyHeight = packet.val;
		}
		else
		{
			sys_err("Someone tried to change other players fly state.");
		}
	}
	else if (packet.type == 5)
	{
		if (p->vid == ch->GetVID()) //igy nem tudnak mas jatekosok sebzest okozni
		{
			...
			{
				std::string arg1 = "0";
				std::string arg2 = std::to_string(static_cast<unsigned int>(ch->GetVID()));
				ch->FlyState = false;
				std::string command = std::string("flystate") + " " + arg1 + " " + arg2;
				ch->ChatPacket(CHAT_TYPE_COMMAND, command.c_str());
				ch->ChatPacket(0, "fly4");
			}
		}
		else
		{
	...
	packet.vid = ch->GetVID();
	ch->PacketAround(&packet, sizeof(TPacketNewMovement), ch);
	//ch->ChatPacket(0, "NewMovementPacket >> PacketAround Done.");
}
#endif
...
void CInputMain::RequestLanguage(LPCHARACTER ch, const char* targetName)
{
	...
	void CInputMain::RedeemPromotionCode(LPCHARACTER ch, const char* c_pData)
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "You have to wait 10 seconds after using promotion codes.");
			return;
		}
