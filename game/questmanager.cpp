...
#define ENABLE_PARTYKILL
	...
	void CQuestManager::SetEventFlag(const std::string& name, int value)
	{
		...
		{
			const DESC_MANAGER::DESC_SET& rkDescSet = DESC_MANAGER::instance().GetClientSet();
			for (auto it = rkDescSet.begin(); it != rkDescSet.end(); ++it)
			{
				const LPCHARACTER pkChar = (*it)->GetCharacter();
				if (pkChar != nullptr)
					pkChar->ChatPacket(CHAT_TYPE_COMMAND, "attendance %d", value);
			}
		}
#endif
#ifdef ENABLE_NEWSTUFF
...
#if defined(__ATTENDANCE_SYSTEM__)
		if ((iEventFlagValue = quest::CQuestManager::instance().GetEventFlag("attendance")))
		{
			ch->ChatPacket(CHAT_TYPE_COMMAND, "attendance %d", iEventFlagValue);
		}
#endif
	}
