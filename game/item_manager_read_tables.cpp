...
bool ITEM_MANAGER::ReadAttendanceGroupFile(const char* c_pszFileName)
{
	for (auto& it : m_pAttendanceGroup)
	{
		if (it.second)
		{
			delete it.second;
	...
	m_pAttendanceGroup.clear();

	CTextFileLoader TextFileLoader;
	if (!TextFileLoader.Load(c_pszFileName))
		return false;

	for (DWORD dwIndex = 0; dwIndex < TextFileLoader.GetChildNodeCount(); ++dwIndex)
	{
		TextFileLoader.SetChildNode(dwIndex);

		std::string strName;
		TextFileLoader.GetCurrentNodeName(&strName);

		BYTE bMonth;
		if (!TextFileLoader.GetTokenByte("month", &bMonth))
		{
			sys_err("ReadAttendanceGroupFile : Syntax error %s : no Month", c_pszFileName);
			TextFileLoader.SetParentNode();
			return false;
		}

		TTokenVector* pToken;
		CAttendanceGroup* pGroup = new CAttendanceGroup(bMonth);

		for (BYTE bDay = 1; bDay < ATTENDANCE_DAYS + 1; ++bDay)
		{
			if (!TextFileLoader.GetTokenVector(std::to_string(bDay), &pToken))
				break;
			...
			if (!ITEM_MANAGER::instance().GetTable(dwVNum))
			{
				sys_err("ReadAttendanceGroupFile : there is no item %s, node: %s", strVNum.c_str(), strName.c_str());
				return false;
			}
			...
			pGroup->AddItem(dwVNum, bCount);
		}

		if (pGroup->GetSize() == 0)
		{
			sys_err("ReadAttendanceGroupFile : there is no item, node: %s", strName.c_str());
			return false;
		}
		else
			m_pAttendanceGroup.emplace(std::make_pair(bMonth, pGroup));
...
const CAttendanceGroup* ITEM_MANAGER::GetAttendanceGroup(BYTE bMonth) 
{
	auto it = m_pAttendanceGroup.find(bMonth);
	return (it != m_pAttendanceGroup.end() ? it->second : nullptr);
}
#endif