...
class CAttendanceGroup
{
public:
	CAttendanceGroup(BYTE bMonth) : m_bMonth(bMonth) {}
	~CAttendanceGroup() {}

	struct SAttendanceItem
	{
		DWORD dwVNum;
		BYTE bCount;
	};

	void AddItem(DWORD dwVNum, BYTE bCount);

	DWORD GetVNum(BYTE bDay) const;
	BYTE GetCount(BYTE bDay) const;
	std::size_t GetSize() const;

private:
	BYTE m_bMonth;
	std::vector<SAttendanceItem> m_vItems;
};
#endif
...
#ifdef DROP_WIKI
		bool	GetDropItem(LPCHARACTER pkChr, std::vector<DWORD> & vec_drop_item);
#endif		
...
#if defined(__ATTENDANCE_SYSTEM__)
public:
	bool ReadAttendanceGroupFile(const char* c_pszFileName);
	const CAttendanceGroup* GetAttendanceGroup(BYTE bMonth);
private:
	std::unordered_map<BYTE, CAttendanceGroup*> m_pAttendanceGroup;
#endif
};
