...
void LogManager::CheatLog(int32_t pid, std::string hostname, std::string reason) {
	Query("INSERT INTO cheat_log (player_id, ip, reason, time) VALUES(%d, '%s', '%s', NOW())", pid, hostname.c_str(), reason.c_str());
}
#endif
