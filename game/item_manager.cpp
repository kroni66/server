...
bool ITEM_MANAGER::GetDropPct(LPCHARACTER pkChr, LPCHARACTER pkKiller, OUT int& iDeltaPercent, OUT int& iRandRange)
{
	...
	int iDropExpPoints = pkKiller->GetPoint(POINT_ITEM_DROP_BONUS);
	if (iDropExpPoints > 0) {
...
void CAttendanceGroup::AddItem(DWORD dwVNum, BYTE bCount)
{
	m_vItems.push_back({ dwVNum, bCount });
}

DWORD CAttendanceGroup::GetVNum(BYTE bDay) const
{
	return m_vItems[bDay].dwVNum;
}

BYTE CAttendanceGroup::GetCount(BYTE bDay) const
{
	return m_vItems[bDay].bCount;
}

std::size_t CAttendanceGroup::GetSize() const
{
