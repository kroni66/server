...
#endif
#if defined(__ATTENDANCE_SYSTEM__)
	char szAttendanceGroupFileName[FILE_NAME_LEN];
	snprintf(szAttendanceGroupFileName, sizeof(szAttendanceGroupFileName),
		"%s/attendance.txt", LocaleService_GetBasePath().c_str());

	sys_log(0, "LoadLocaleFile: AttendanceGroup: %s", szAttendanceGroupFileName);
	if (!ITEM_MANAGER::instance().ReadAttendanceGroupFile(szAttendanceGroupFileName))
	{
		sys_err("cannot load AttendanceGroup: %s", szAttendanceGroupFileName);
		thecore_shutdown();
		return;
	}
#endif
...
int CInputDB::Analyze(LPDESC d, BYTE bHeader, const char * c_pData)
{
	...
	case HEADER_DG_RESPOND_CHANNELSTATUS:
		...
		break;
#if defined(__ATTENDANCE_SYSTEM__)
	case HEADER_DG_ATTENDANCE_LOAD:
		LoadAttendance(DESC_MANAGER::instance().FindByHandle(m_dwHandle), c_pData);
		break;
#endif
#ifdef __ENABLE_NEW_OFFLINESHOP__
...
#if defined(__ATTENDANCE_SYSTEM__)
void CInputDB::LoadAttendance(const LPDESC pkDesc, const char* c_pszData)
{
	if (pkDesc == nullptr)
		return;

	const LPCHARACTER pkChar = pkDesc->GetCharacter();
	if (pkChar == nullptr)
		return;
	...
	{
		CAttendance::Create(pkChar, (TAttendanceTable*)c_pszData);
		return;
	}

	pAttendance->Load((TAttendanceTable*)c_pszData);
}
#endif
